...
from ..dataset.voc import class_names
import pickle
...
def _serialize_to_tensor(data, group):
    ...
    tensor = torch.ByteTensor(storage).to(torch.device('cpu'))
    return tensor
...
class PascalVOCDetectionEvaluator:
    ...
    def process(self, targets, outputs):
        for i in range(outputs[0].shape[0]):
            ...
            boxes = outputs[1][i].to(self._cpu_device).data.numpy()
            scores = outputs[0][i].to(self._cpu_device).data.numpy()
            classes = outputs[2][i].to(self._cpu_device).data.numpy()
