...
class MultiheadAttention(nn.Module):
    def __init__(self, hid_dim, n_heads, dropout=0.0):
        ...
        self.in_proj_weight = nn.Parameter(torch.Tensor(3*hid_dim, hid_dim))
        self.in_proj_bias = nn.Parameter(torch.Tensor(3*hid_dim))
    ...
    def forward(self, query, key, value, mask=None):
        assert query.float == key.float
        assert query.float == value.float
        tgt_len, bsz, embed_dim = query.size()
        head_dim = embed_dim // self.n_heads

        Q, K, V = torch.nn.functional.linear(query, self.in_proj_weight, self.in_proj_bias).chunk(3, dim=-1)

        scaling = float(head_dim) ** -0.5
        Q = Q * scaling

        Q = Q.contiguous().view(tgt_len, bsz * self.n_heads, head_dim).transpose(0, 1)
        K = K.contiguous().view(tgt_len, bsz * self.n_heads, head_dim).transpose(0, 1)
        V = V.contiguous().view(tgt_len, bsz * self.n_heads, head_dim).transpose(0, 1)

        attn_output_weights = torch.bmm(Q, K.transpose(1, 2))

        attention = self.do(torch.softmax(attn_output_weights, dim=-1))
