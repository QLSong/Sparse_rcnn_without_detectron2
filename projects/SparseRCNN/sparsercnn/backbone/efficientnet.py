...
class EfficientNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            ...
            if prev_x.size(2) > x.size(2):
                endpoints['res{}'.format(len(endpoints)+1)] = prev_x.contiguous()
            prev_x = x

        # Head
        x = self._swish(self._bn1(self._conv_head(x)))
        endpoints['res{}'.format(len(endpoints)+1)] = x.contiguous()
    ...
    def output_shape(self):
        return {'res2': ShapeSpec(stride=4, channels=24), 
                'res3': ShapeSpec(stride=8, channels=40), 
                'res4': ShapeSpec(stride=16, channels=112), 
                'res5': ShapeSpec(stride=32, channels=1280)}
